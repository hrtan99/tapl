...
| "*/" { errAt (info lexbuf) "Unmatched end of comment" }
...
| eof
    { errAt (!startLex) "Comment not terminated" }
| [^ '\n']
    { comment lexbuf }
| "\n"
...
| eof  { errAt (!startLex) "String not terminated" }
...
| ['0'-'9']['0'-'9']['0'-'9']
    {
      ...
      if x > 255 then
	errAt (info lexbuf) "Illegal character constant"
      else
	Char.chr x
    }
| [^ '"' '\\' 't' 'n' '\'']
